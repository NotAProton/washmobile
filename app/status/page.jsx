...
function Day ({ day, data }) {
  ...
  return (
    <div className='block m-0.5 sm:m-1 my-2'>
      <Container className="p-1 items-center text-center rounded-lg pb-2" style={{ background: '#4b4358' }}>
...
function ButtonMod ({ item }) {
  ...
  return (
    <Button disabled={item.status !== 'booked'} size={'lg'} className={'block mt-2 rounded-md'}
      style={{
        backgroundColor: item.status === 'booked' ? '#25202d' : '#3f384c',
        width: '100%',
        color: item.status === 'booked' ? 'rgb(249,249,249)' : '#cdc2db'
      }}
...
function StatusWidget ({ data }) {
  ...
  return (
    <div className='flex mb-2 mt-1'>
      <Container className='w-2/3 inline mr-0 rounded-l-lg' style={{ background: '#4b4358', color: '#eadef7' }}>
      ...
      <Container backgroundColor='#4b4358' className='ml-0 w-1/3 inline px-px rounded-r-lg' style={{ background: '#4b4358', color: '#eadef7' }}>
...
function Navbar () {
  return (
    <nav className='flex mx-auto max-w-screen-xl px-4 py-3 rounded-b-lg p-0 align-middle' style={{ color: '#e6e1e6' }}>
      <div className="text-blue-gray-900 text-2xl sm:text-4xl my-auto" >
        MJA Wash
      </div>
      <div className="ml-auto">
        <Link href='/book'>
          <Button size='large' className={'p-3 bg-violet-800 hover:bg-violet-900 block rounded-md'}
          style={{
            width: '100%',
            color: '#3d1b73',
            background: '#d4bbff'
...
function SlotModal () {
  ...
  return (
    <Modal centered={true} opened={opened} onClose={() => dispatch(closeModal())} title={<>{slot.label} ({getDateFromSlotID(slotno)})</>}>
      {slot.status === 'booked'
        ? <><span style={{ color: '#ff6959', fontWeight: 'bold' }} className='font-medium'>Reserved <br /></span> <span className='text-lg'>{formatName(slot.bookedBy)} </span></>
        : 'Unreserved'}</Modal>
  )
}
